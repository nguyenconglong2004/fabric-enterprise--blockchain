...
- Node **tự thêm chính mình** vào membership: `Membership.AddMember(selfID, joinTime)`
- Sau `Start()`, node ở trạng thái **Follower** với `currentLeaderID = ""`. Node **không** tự lên Leader.
- Để trở thành Leader hoặc Follower cần gọi tường minh:
  - `BootstrapAsLeader()` — khi là node đầu tiên của cluster mới.
  - `JoinCluster(addr)` — khi cluster đã tồn tại; tự query leader rồi gửi join request.
...
- Node **không** tự lên Leader ở bước này. Caller phải gọi `BootstrapAsLeader()` hoặc `JoinCluster()` sau `Start()`.

### (2) Node mới join cluster — qua `JoinCluster`

Files: `internal/raft/node.go`, `internal/raft/membership.go`, `internal/types/message.go`

**Bên node join (B)**:

- **Khi**: `JoinCluster(bootstrapAddr)` — sau `Start()`
- Kết nối tới bootstrap peer (có thể là Follower)
- Gửi `MsgMembershipRequest` để hỏi leader của cluster
- Chờ `MsgMembershipResponse` (chứa `leader_id` + `members[]`)
- Nếu `leader_id` rỗng (đang bầu chọn) → retry tối đa 5 lần với 2s backoff
- Nếu leader là peer khác → nạp leader addresses vào peerstore
- Gửi join request bằng `MsgMembershipUpdate` (payload `MembershipProposal`) **trực tiếp tới leader**
...
> **Lý do không gửi thẳng join request tới bootstrap peer:** Nếu bootstrap peer là Follower, `handleMembershipUpdate()` sẽ drop im lặng (`if !IsLeader() { return }`). `JoinCluster` tránh vấn đề này bằng cách query leader trước. Xem chi tiết: [TC05](scenarios/tc05-phantom-leader-on-join.md).
