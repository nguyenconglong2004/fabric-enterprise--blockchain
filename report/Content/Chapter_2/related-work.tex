...
Cốt lõi của mọi blockchain là một \textbf{sổ cái phân tán}: một danh sách các
...
tính bằng một \textbf{hàm băm mật mã} (cryptographic hash function) --- một hàm
...
khiến dữ liệu trở nên gần như \textbf{bất biến}. Đây chính là cơ chế ``chuỗi''
...
blockchain sử dụng \textbf{cây Merkle}~\cite{merkle1979} (Merkle tree, hay hash
...
đáo: mô hình \textbf{UTXO} (Unspent Transaction Output --- đầu ra giao dịch chưa
tiêu).
...
\begin{itemize}
  \item Các \textbf{đầu vào} (inputs, ký hiệu \texttt{Vin}): tham chiếu và
  ``tiêu'' các UTXO đã tồn tại từ trước.
  \item Các \textbf{đầu ra} (outputs, ký hiệu \texttt{Vout}): tạo ra các UTXO
...
trạng thái phức tạp. Hệ thống của khóa luận \textbf{hỗ trợ mô hình UTXO}: world
...
khả năng lập trình. Đóng góp lớn nhất của Ethereum là \textbf{hợp đồng thông
minh} (smart contract): các chương trình được lưu và thực thi trên blockchain,
tự động chạy khi có giao dịch gọi tới.

Ethereum sử dụng \textbf{mô hình tài khoản} (account-based model) thay cho UTXO:
...
và thực thi theo mô hình \textbf{Order-Execute}: các giao dịch được sắp xếp
...
cấp phép tiêu biểu cho doanh nghiệp, và là \textbf{nguồn cảm hứng kiến trúc
chính} của khóa luận. Đóng góp quan trọng nhất của Fabric --- so với mô hình
Order-Execute của Ethereum --- là kiến trúc \textbf{Execute--Order--Validate},
...
\begin{enumerate}
  ...
  \textbf{chưa} ghi vào sổ cái. Vì các peer thực thi độc lập, bước này có thể
  được song song hóa.
  ...
  thành block. Đáng chú ý, ordering service \textbf{không cần hiểu nội dung}
...
Mô hình này mang lại ba lợi ích cốt lõi: (a)~\textbf{khả năng song song} ở pha
thực thi; (b)~\textbf{tính module hóa}, cho phép thay thế cơ chế đồng thuận mà
không ảnh hưởng tới logic nghiệp vụ; và (c)~\textbf{bảo mật theo lớp}, vì mỗi
tầng chỉ thấy thông tin nó cần. Fabric thật sử dụng thư viện
etcd/raft cho ordering service.

Hệ thống của khóa luận \textbf{tái hiện trung thành} triết lý này: Core Service
...
\textbf{tự cài đặt thuật toán Raft từ đầu} thay vì dùng thư viện, và đơn giản
...
\begin{itemize}
  ...
  chuẩn dùng \textbf{timeout ngẫu nhiên}: Follower nào hết giờ chờ trước sẽ trở
  ...
  tới \textbf{đa số} (majority) node, nó được coi là đã \textit{commit}.
...
Khái niệm \textbf{quorum đa số} ($\lfloor N/2 \rfloor + 1$ trên tổng $N$ node)
...
\textbf{chịu lỗi dừng} (Crash Fault Tolerant --- CFT), giả định các node không
có hành vi độc hại, chỉ có thể dừng hoặc chậm.

Khóa luận \textbf{tự cài đặt Raft} nhưng thay cơ chế bầu cử bằng timeout ngẫu
nhiên bằng một biến thể \textbf{bầu lãnh đạo theo độ ưu tiên} (xem
...
toán \textbf{chịu lỗi Byzantine} (Byzantine Fault Tolerant --- BFT).
...
\textbf{WebAssembly} (WASM)~\cite{haas2017wasm,wasmspec} là một định dạng mã máy
...
WASM phía máy chủ thường dùng \textbf{WASI} (WebAssembly System
...
\textbf{TinyGo}, và thực thi bằng \textbf{wazero} --- một runtime WASM thuần Go
...
\textbf{libp2p}~\cite{libp2p} --- ngăn xếp mạng module hóa được tách ra từ dự án
IPFS~\cite{benet2014ipfs} và cũng là nền mạng của Ethereum 2.0. libp2p cung cấp:
(a)~\textbf{định danh mật mã} (PeerID suy ra từ khóa công khai); (b)~\textbf{địa
chỉ tự mô tả} (multiaddr) như \texttt{/ip4/127.0.0.1/tcp/6000/p2p/12D3Koo...};
(c)~\textbf{ghép kênh} (stream multiplexing) cho phép nhiều ``làn'' dữ liệu song
song trên một kết nối; và (d)~\textbf{thương lượng giao thức} qua các Protocol
...
\textbf{LevelDB}~\cite{leveldb} là một thư viện lưu trữ key-value nhúng, chạy
...
blockchain, LevelDB hỗ trợ \textbf{ghi theo lô nguyên tử} (atomic batch): nhiều
...
giao dịch tích lũy thành \textbf{backlog} và độ trễ tăng vọt --- hiện tượng có
thể giải thích bằng \textbf{định luật Little}~\cite{little1961law}: số lượng
...
vận hành, mà hướng tới việc \textbf{làm chủ tri thức nền tảng} thông qua việc tự
