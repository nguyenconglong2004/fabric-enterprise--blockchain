...
Khóa luận đã tìm hiểu, thiết kế và hiện thực từ đầu một nền tảng
...
Execute--Order--Validate qua bốn tiến trình độc lập, với các đóng góp
...
bằng một thuật toán chịu lỗi Byzantine (BFT). Do kiến trúc đã tách bạch
...
Bổ sung write-ahead log có fsync cho log Raft, kèm cơ chế
snapshot và nén log để tránh log tăng vô hạn và rút ngắn thời gian phục
...
nhiều block gối đầu trước khi block trước đó được commit, tương tự cơ
...
là tối ưu đường Core: (i)~song song hoá xác minh chữ ký endorsement và
gộp lô kiểm tra; (ii)~xem xét chữ ký tổng hợp (ví dụ BLS) để giảm số
...
Lặp lại bộ thực nghiệm ở Chương~\ref{chap:evaluation} trên cụm nhiều
...
Hỗ trợ thêm/bớt node và điều chỉnh độ ưu tiên trong lúc cluster đang
...
Bổ sung một SDK phát triển hợp đồng đa ngôn ngữ (mọi ngôn ngữ biên dịch
được sang WASM), cơ chế vòng đời hợp đồng (cài đặt, nâng cấp, thu hồi có
kiểm soát), và đo/giới hạn tài nguyên (gas metering) để chống hợp đồng
